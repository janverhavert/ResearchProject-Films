async function postData() {
  ...
  if (Password == Password2) {
    ...
    var formData = JSON.stringify({ UserName: username, Password: Password, Email: email });
    var requestOptions = {
      method: 'POST',
      headers: myHeaders,
      body: formData,
    ...
    fetch('http://localhost:63436/api/auth/register', requestOptions)
...
const AddUserToRole = async (userId) => {
  var myHeaders = new Headers();
  myHeaders.append('Content-Type', 'application/json');
  myHeaders.append(
    'Cookie',
  ...
  var formData = JSON.stringify({ UserId: userId, RoleId: '8b2ed6ff-54c6-4647-89cc-cf21860396d5' });
  var requestOptions = {
    method: 'POST',
    headers: myHeaders,
    body: formData,
  ...
  await fetch('http://localhost:63436/api/auth/', requestOptions)
    .then((response) => response.text())
